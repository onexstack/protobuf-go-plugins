...
// addImport records that the file generated for f has to import path.
//
// Keyed by file rather than module-wide, and the difference is not cosmetic.
// pgs renders a registered template only after every target file has been
// walked, so the "imports" template function reads this state at render time,
// not at registration time. One shared set therefore hands every generated file
// the union of all files' imports, and a file that does not reference a package
// gets an import for it -- which is a compile error, not a warning.
//
// The key is the file path, not FullyQualifiedName: two files in the same proto
// package share a qualified name, so the latter would collide and reintroduce
// exactly the sharing this function exists to prevent.
func (m *Module) addImport(f pgs.File, path string) {
	m.importsFor(f)[path] = struct{}{}
}

func (m *Module) importsFor(f pgs.File) map[string]struct{} {
	key := f.Name().String()
	set, ok := m.imports[key]
	if !ok {
...
func (m *Module) InitContext(c pgs.BuildContext) {
	...
	tpl := template.New("fields").Funcs(map[string]interface{}{
		...
		"imports": func(f pgs.File) string {
			var imports string
			for v := range m.imports[f.Name().String()] {
