...
{{- else -}}
{{/* (validate.unexported): the message's exported pair is hand-written, so the
     generated entry points take the underscore-prefixed names instead. The rule
     body, the error types and the lookup tables below are unchanged -- only
     these two names move, which is what lets a hand-written Validate call
     through and add checks of its own without losing the compiled rules. */}}
{{ cmt "_Validate runs the rules compiled from " (msgTyp .) " and returns the first violation, or nil. It is the generated half of the pair; the exported Validate is hand-written and calls this." -}}
func (m {{ (msgTyp .).Pointer }}) _Validate() error {
	return m.validate(false)
}

{{ cmt "_ValidateAll runs the rules compiled from " (msgTyp .) " and returns every violation wrapped in " (multierrname .) ", or nil if none found. It is the generated half of the pair; the exported ValidateAll is hand-written and calls this." -}}
func (m {{ (msgTyp .).Pointer }}) _ValidateAll() error {
	return m.validate(true)
}
{{- end }}
